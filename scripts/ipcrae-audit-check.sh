#!/bin/bash
# Audit réel du vault IPCRAE — score dynamique 0-65
# Usage: ipcrae-audit-check [--verbose] [--fast]
#   --fast : mode incrémental — vérifie uniquement sections 1+4+9 si dernier audit < 1h et score > 50
...
audit_section6() {
  ...
  local scripts_ok=1
  for script in ipcrae ipcrae-audit-check ipcrae-auto ipcrae-auto-apply ipcrae-auto-core ipcrae-inbox-scan ipcrae-moc-auto ipcrae-agent-bootstrap; do
...
# ══════════════════════════════════════════════
audit_section9() {
  section_header "Section 9 — Conformité Agent (5 pts)"
  local s=0

  # 9.1 Bootstrap loggué < 24h (2 pts — CRITIQUE)
  local bootstrap_log="${IPCRAE_ROOT}/.ipcrae/auto/bootstrap-log.txt"
  local age_h
  age_h=$(file_age_hours "$bootstrap_log")
  if [[ "$age_h" -le 24 ]]; then
    check_line ok "Bootstrap session loggué (dernière session < 24h)" 2 2
    s=$(( s + 2 ))
  else
    check_line ko "Pas de bootstrap session < 24h" 0 2 \
      "Lancer: ipcrae start --project <slug> pour valider le contexte"
    CRITIQUES=$(( CRITIQUES + 1 ))
  fi

  # 9.2 Notes récentes (7j) sans tags majuscules (2 pts — IMPORTANT)
  local recent_bad=0
  while IFS= read -r f; do
    if awk '/^---$/{fm++; next} fm==1 && /^tags:/{print; exit} fm>=2{exit}' \
       "$f" 2>/dev/null | grep -qE '[A-Z]'; then
      recent_bad=$(( recent_bad + 1 ))
    fi
  done < <(find "$IPCRAE_ROOT" -name "*.md" -mtime -7 \
    ! -path "*/.git/*" ! -path "*/Archive*" ! -path "*/traites/*" 2>/dev/null)
  if [[ "$recent_bad" -eq 0 ]]; then
    check_line ok "Notes récentes (7j) : frontmatter tags lowercase" 2 2
    s=$(( s + 2 ))
  else
    check_line ko "Notes récentes (7j) : ${recent_bad} note(s) avec tags majuscules" 0 2 \
      "Corriger les tags en minuscules dans les notes récentes"
    IMPORTANTS=$(( IMPORTANTS + 1 ))
  fi

  # 9.3 Workflow GTD : pas de bypass permanents/ < 24h (1 pt — MINEUR)
  local direct_perm
  direct_perm=$(find "${IPCRAE_ROOT}/Zettelkasten/permanents" \
    -name "*.md" -mmin -1440 2>/dev/null | wc -l | tr -d ' \t')
  if [[ "${direct_perm:-0}" -eq 0 ]]; then
    check_line ok "Workflow GTD : aucun bypass permanents/ (24h)" 1 1
    s=$(( s + 1 ))
  else
    check_line ko "Workflow GTD : ${direct_perm} note(s) directes dans permanents/ (<24h)" 0 1 \
      "Déplacer vers Zettelkasten/_inbox/ pour maturation"
    MINEURS=$(( MINEURS + 1 ))
  fi
...
main() {
  ...
  # Mode --fast : sections critiques uniquement si dernier audit < 1h et score > 50
  local _fast_ok=false
  if [[ "$FAST_MODE" == "true" ]]; then
    local _last_audit="${IPCRAE_ROOT}/.ipcrae/cache/last-audit.txt"
    if [[ -f "$_last_audit" ]]; then
      local _last_ts _last_score _elapsed
      _last_ts=$(grep '^timestamp=' "$_last_audit" | cut -d= -f2)
      local _last_pct
      _last_pct=$(grep '^pct=' "$_last_audit" | cut -d= -f2)
      _elapsed=$(( $(date +%s) - ${_last_ts:-0} ))
      if [[ "$_elapsed" -le 3600 && "${_last_pct:-0}" -gt 75 ]]; then
        _fast_ok=true
        echo -e "${YELLOW}[--fast] Dernier audit < 1h, score ${_last_pct}% > 75% — sections critiques uniquement (1, 4, 9)${NC}"
      fi
    fi
    if [[ "$_fast_ok" == "false" ]]; then
      echo -e "${YELLOW}[--fast] Conditions non remplies — audit complet lancé${NC}"
    fi
  fi

  if [[ "$_fast_ok" == "true" ]]; then
    audit_section1   # 9 pts
    audit_section4   # 13 pts
    audit_section9   # 5 pts
    # En mode fast, MAX_SCORE effectif = 27 (sections 1+4+9)
    MAX_SCORE=27
  else
    audit_section1
    audit_section2
  ...
  # Cache last-audit pour le mode --fast
  local cache_dir="${IPCRAE_ROOT}/.ipcrae/cache"
  mkdir -p "$cache_dir"
  printf '%s\n' \
    "timestamp=$(date +%s)" \
    "score=${TOTAL_SCORE}" \
