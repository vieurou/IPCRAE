...
# ═══════════════════════════════════════════════════════════════════════════
section "Lanceur CLI"
if prompt_yes_no "Installer ~/bin/ipcra (depuis ipcra-launcher.sh) ?" "y"; then
  SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  LAUNCHER_SRC="${SCRIPT_DIR}/ipcra-launcher.sh"

  if [ ! -f "$LAUNCHER_SRC" ]; then
    logerr "ipcra-launcher.sh introuvable dans $SCRIPT_DIR"
    logerr "Place ipcra-launcher.sh à côté de ipcra-install.sh"
    exit 1
  fi

  mkdir -p "$HOME/bin"

  # Remplacer IPCRA_ROOT dans le lanceur
  sed "s|IPCRA_ROOT=\"\${IPCRA_ROOT:-\${HOME}/IPCRA}\"|IPCRA_ROOT=\"${IPCRA_ROOT}\"|" \
    "$LAUNCHER_SRC" > "$HOME/bin/ipcra"
  chmod +x "$HOME/bin/ipcra"
  loginfo "Lanceur installé: ~/bin/ipcra"

  # Raccourcis par provider
  for p in claude gemini codex; do
    printf '#!/usr/bin/env bash\nexec "$HOME/bin/ipcra" -p %s "$@"\n' "$p" > "$HOME/bin/ipcra-${p}"
    chmod +x "$HOME/bin/ipcra-${p}"
    loginfo "Raccourci: ~/bin/ipcra-${p}"
  done
  ...
  case ":$PATH:" in
    *:"$HOME/bin":*) ;;
    *)
      logwarn "~/bin n'est pas dans le PATH."
      local rcfile="$HOME/.bashrc"
      case "$SHELL" in
        */zsh)  rcfile="$HOME/.zshrc" ;;
        */fish) rcfile="$HOME/.config/fish/config.fish" ;;
      esac
      if prompt_yes_no "Ajouter dans $(basename "$rcfile") ?" "y"; then
        printf '\nexport PATH="$HOME/bin:$PATH"\n' >> "$rcfile"
        loginfo "Ajouté à $rcfile. Faire: source $rcfile"
      fi ;;
  esac
fi
...
cat <<'CMDS'
  ipcra               # menu interactif
  ipcra daily         # daily note (Obsidian ou $EDITOR)
  ipcra daily --prep  # daily pré-rédigée par l'IA
  ipcra weekly        # weekly ISO
  ipcra monthly       # revue mensuelle
  ipcra close         # clôture session (maj mémoire domaine)
  ipcra sync          # régénère fichiers provider
  ipcra zettel "X"    # créer note Zettelkasten
  ipcra moc "X"       # créer/ouvrir Map of Content
  ipcra health        # diagnostic système
  ipcra DevOps        # mode expert DevOps
  ipcra -p gemini     # provider explicite
  ipcra-claude        # raccourci Claude
  ipcra-gemini        # raccourci Gemini
  ipcra-codex         # raccourci Codex
CMDS
printf '\n%bProviders :%b Claude → CLAUDE.md | Gemini → GEMINI.md | Codex → AGENTS.md | Kilo → .kilocode/rules/\n' "$BOLD" "$NC"
printf '\n%bMéthodologie :%b\n' "$BOLD" "$NC"
printf '  1. Définir ta Phase active dans Phases/index.md\n'
printf '  2. Chaque matin : ipcra daily (ou daily --prep)\n'
printf '  3. Chaque dimanche : ipcra weekly\n'
printf '  4. En fin de session IA : ipcra close\n'
printf '  5. Notes de pensée : ipcra zettel "titre"\n'
printf '  6. Modifier .ipcra/context.md + instructions.md puis ipcra sync\n'
